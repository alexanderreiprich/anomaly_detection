...
class PatientModel:
    """XGBoost wrapper for severity classification.

    XGBoost is scale-invariant and handles NaN natively — features may be
    missing (e.g. IPSS without a nearby submission, or NO_DATA biomarkers).
    The feature set is configurable so the same wrapper serves both the
    uroflow and biomarker models; it is persisted alongside the model.
    """
    ...
    def __init__(
        ...
        self.trained = False
        self.features: list[str] = list(features) if features else list(FEATURES)
